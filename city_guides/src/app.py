...
from city_guides.src.marco_response_enhancer import analyze_user_intent, should_call_groq
...
cors(
    app,
    allow_origin=["http://localhost:5174", "https://travelland-w0ny.onrender.com"],
...
US_STATE_ICONS = {
    ...
    "Wisconsin": "🧀",
    "Wyoming": "🐎",
    "District of Columbia": "🏛️",
}


async def fetch_city_wikipedia(
        ...
        async with aiohttp.ClientSession() as session:
            for title in _candidates():
                ...
                async with session.get(
                    "https://en.wikipedia.org/w/api.php", params=params, timeout=6
                ) as resp:
                    if resp.status != 200:
                        continue
                    ...
                    if isinstance(data, list) and len(data) >= 2 and data[1]:
                        ...
                        try:
                            ...
                            app.logger.exception(
                                "OpenSearch Wikipedia summary failed for %s via %s",
                                city,
                                best_title,
                            )
    except Exception:
...
try:
    ...
    async def ddgs_search(*args, **kwargs):
        app.logger.warning(
            "DDGS provider unavailable (%s); returning empty results", ddgs_import_err
        )
        return []

    app.logger.debug(
        "DDGS provider not available at module import time; falling back to empty search results"
    )

from city_guides.groq.traveland_rag import TravelLandRecommender, recommender
...
async def api_chat_rag():
    ...
    try:
        ...
        is_fun_fact_query = has_direct_pattern or (
            has_fact_intent and any(kw in query.lower() for kw in ["fact", "thing", "special"])
        )
        ...
        web_results = []
        try:
            web_results = await ddgs_search(
                full_query, engine=engine, max_results=max_results, timeout=DEFAULT_DDGS_TIMEOUT
            )
        except Exception:
        ...
        if not neighborhood_from_query:
            ...
            match = re.search(
                r"in\s+(la\s+|el\s+|les\s+|els\s+)?([^,]+?)(?:\s+in|\s+near|\s+area)?\s*$",
                query,
                re.IGNORECASE,
            )
            if match:
        ...
        user_prompt = (
            f"User query: {query}{location_fragment}\n\nRelevant web snippets:\n{context_text}"
        )
            ...
            if raw:
                try:
                    ...
                    if isinstance(parsed, list) and len(parsed) == 0:
                        ...
                        app.logger.debug(
                            "Empty cached neighborhoods for %s; treating as miss", cache_key
                        )
                    else:
                    ...
                    app.logger.debug(
                        "Failed to parse cached neighborhoods for %s; refetching", cache_key
                    )
        except Exception:
            ...
            if result:
                ...
                try:
                    data = await multi_provider.async_get_neighborhoods(
                        city=None, lat=g_lat, lon=g_lon, lang=lang, session=aiohttp_session
                    )
                    if data:
                        app.logger.info(
                            "Geocode fallback succeeded for '%s' -> %s,%s (%d items)",
                            city,
                            g_lat,
                            g_lon,
                            len(data),
                    ...
                    app.logger.exception(
                        "neighborhoods fetch failed on geocode fallback for %s", city
                    )
        except Exception:
...
async def reverse_lookup():
    ...
    try:
        from city_guides.providers.overpass_provider import (
            async_reverse_geocode,
            geoapify_reverse_geocode_raw,
        )
        ...
        if props:
            raw_geoapify = props
            app.logger.debug(
                "geoapify_reverse_geocode_raw returned properties: %s",
                {k: props.get(k) for k in ["country_code", "country", "state", "city"]},
            )
    ...
    if not country_code:
        try:
            ...
            for c in countries:
                ...
                if country_name and (
                    country_name.lower() in cname or cname in country_name.lower()
                ):
    ...
    if not addr and not country_code and not country_name:
        app.logger.warning(
            "reverse_lookup failed to derive any location info for coords %s,%s", lat, lon
        )
        return (
            jsonify(
                {
                    "error": "reverse_lookup_failed",
    ...
    nb = []
    try:
        nb = await multi_provider.async_get_neighborhoods(
            city=None, lat=float(lat), lon=float(lon), lang="en", session=aiohttp_session
        )
    except Exception:
            ...
            nb_norm.append(
                {
                    ...
                    "name": n.get("name") or n.get("display_name") or n.get("label") or n.get("id"),
                }
            )
        except Exception:
            continue
    ...
    if debug_flag:
        response["debug"] = {"geoapify_props": raw_geoapify, "nominatim_addr": raw_nominatim}
...
async def smoke():
    ...
    try:
        ...
        try:
            nb = await multi_provider.async_get_neighborhoods(
                city=None, lat=lat, lon=lon, lang="en", session=aiohttp_session
            )
        ...
        async with get_session() as session:
            ...
            async with session.get(country_url, params=country_params, timeout=10) as resp:
                ...
                for country in country_data["geonames"]:
                    ...
                    if (
                        country.get("countryCode", "").upper() == country_code.upper()
                        or country.get("name", "").lower() == country_code.lower()
                    ):
                ...
                async with session.get(
                    children_url, params=children_params, timeout=10
                ) as children_resp:
            ...
            if seed_path.exists():
                ...
                if country_code:
                    cities_data = [
                        ...
                        if (c.get("countryCode") or "").upper() == country_code.upper()
                    ]
                if state_code:
                    cities_data = [
                        c
                        for c in cities_data
                        if (c.get("stateCode") or "").upper() == state_code.upper()
                    ]
                ...
                for c in cities_data:
                    key = (
                        ...
                        (c.get("countryCode") or "").upper(),
                        (c.get("stateCode") or "").upper(),
                    )
                    if key in seen:
                        continue
                    seen.add(key)
                ...
                app.logger.info(
                    "Serving %d unique cities from seeded_cities.json fallback", len(cities)
                )
    ...
    try:
        async with get_session() as session:
            ...
            async with session.get(cities_url, params=cities_params, timeout=10) as resp:
                if resp.status != 200:
                    app.logger.warning(
                        "GeoNames returned status %d for %s/%s; trying seeded fallback",
                        resp.status,
                        country_code,
                        state_code,
                    )
                ...
                if not cities:
                    app.logger.info(
                        "GeoNames returned no cities for %s/%s; falling back to seeded_cities.json",
                        country_code,
                        state_code,
                    )
                    seed_path = Path(__file__).parent.parent / "data" / "seeded_cities.json"
...
async def api_neighborhoods():
    ...
    try:
        ...
        async with aiohttp_session.get(
            f"http://localhost:5010/neighborhoods?city={city_name}&lang=en"
        ) as resp:
            if resp.status == 200:
                data = await resp.json()
...
async def get_fun_fact():
    ...
    try:
        ...
        if not seeded_facts:
            try:
                ...
                try:
                    ...
                    ddgs_results = await ddgs_search(
                        f"interesting facts about {city}",
                        max_results=5,
                        timeout=float(os.getenv("DDGS_TIMEOUT", "5")),
                    )
                    fun_facts_from_ddgs = []
                    for r in ddgs_results:
                        ...
                        for s in sentences:
                            if 50 < len(s) < 180 and re.search(
                                r"\d|largest|oldest|first|only|famous|world", s.lower()
                            ):
                    ...
                    if wiki_text:
                        ...
                        sentences = [
                            s.strip() for s in wiki_text.split(".") if 40 < len(s.strip()) < 200
                        ]
                        # Skip boring definitions
                        filtered = [
                            s
                            for s in sentences
                            if not re.match(r"^\w+ is a (city|town|commune)", s.lower())
                        ]
        ...
        return jsonify({"city": city, "funFact": selected_fact})
        ...
        try:
            ...
            if seed_path.exists():
                ...
                if city_key:
                    ...
                    app.logger.info(
                        f"Found {len(seed_neighborhoods)} neighborhoods in seed file for {city}"
                    )
        except Exception as e:
    ...
    try:
        ...
        if WIKI_CITY_AVAILABLE and isinstance(result, dict):
            ...
            if not has_quick and not has_summary:
                try:
                    wiki_data = await fetch_city_wikipedia(
                        city, state_name or None, country_name or None
                    )
                    if wiki_data:
    ...
    try:
        ...
        for venue in venues[:10]:  # Limit to 10 venues
            synthesized_venue = {
                ...
                or f'A popular {search_result.get("category", "spot")} in {search_result.get("city", "the city")}',
                "lat": venue.get("lat"),
                "lon": venue.get("lon"),
                "provider": venue.get("provider", "osm"),
                "tags": venue.get("tags", {}),
                "enhanced_description": venue.get("description")
                or f'Great option for {search_result.get("category", "dining")} in the area',
            }
            synthesized_venues.append(synthesized_venue)

        return jsonify({"synthesized_venues": synthesized_venues, "total": len(synthesized_venues)})
...
async def generate_quick_guide(skip_cache=False, disable_quality_check=False):
        ...
        try:
            ...
            if not disable_quality_check:
                ...
                try:
                    ...
                    if src in ("ddgs", "synthesized") and (
                        looks_like_ddgs_disambiguation_text(raw_q) or "missing:" in raw_q.lower()
                    ):
                        app.logger.info(
                            "Replacing raw cached %s quick_guide for %s/%s due to disambiguation/promotional content (early replacement)",
                            src,
                            city,
                            neighborhood,
                        )
                        try:
                            ...
                            new_para = SynthesisEnhancer.generate_neighborhood_paragraph(
                                neighborhood, city
                            )
                            ...
                            try:
                                ...
                                app.logger.exception(
                                    "Failed to persist synthesized replacement for cached disambiguation (early)"
                                )
                        except Exception:
                            app.logger.exception(
                                "Failed to synthesize replacement for cached disambiguation (early)"
                            )
                            try:
                                cache_file.unlink()
                            except Exception:
                ...
                try:
                    ...
                    resp["quick_guide"] = SynthesisEnhancer.neutralize_tone(
                        resp.get("quick_guide") or "",
                ...
                try:
                    if resp.get("source") == "data-first" and re.match(
                        r"^.+ is a neighborhood in .+\.$", (resp.get("quick_guide") or "")
                    ):
                        try:
                            from city_guides.src.geo_enrichment import (
                                build_enriched_quick_guide,
                                enrich_neighborhood,
                            )

                            enrichment = await enrich_neighborhood(
                                city, neighborhood, session=aiohttp_session
                            )
                            if enrichment and (
                                enrichment.get("text")
                                or (enrichment.get("pois") and len(enrichment.get("pois")) > 0)
                            ):
                                resp["quick_guide"] = build_enriched_quick_guide(
                                ...
                                try:
                                    ...
                                    app.logger.exception(
                                        "Failed to persist geo-enriched replacement for cached entry"
                                    )
                                return jsonify(resp)
                        except Exception:
            ...
            if src in ("ddgs", "synthesized") and (
                looks_like_ddgs_disambiguation_text(quick_text) or "missing:" in quick_text.lower()
            ):
                app.logger.info(
                    "Replacing cached %s quick_guide for %s/%s due to disambiguation/promotional content",
                    src,
                    city,
                    neighborhood,
                )
                try:
                    ...
                    try:
                        ...
                        app.logger.exception(
                            "Failed to persist synthesized replacement for cached disambiguation"
                        )
                    return jsonify(resp)
                except Exception:
                    app.logger.exception(
                        "Failed to synthesize replacement for cached disambiguation"
                    )
                    try:
                        cache_file.unlink()
                    except Exception:
            ...
            try:
                if data.get("source") == "wikipedia":
                    ...
                    if any(b in text for b in blacklist) and (
                        city.lower() not in text and neighborhood.lower() not in text
                    ):
                        app.logger.info(
                            "Ignoring cached wikipedia quick_guide for %s/%s due to likely unrelated event/article",
                            city,
                            neighborhood,
                        )
                    else:
    ...
    try:
        is_valid, confidence, suggested = NeighborhoodDisambiguator.validate_neighborhood(
            neighborhood, city
        )
        if not is_valid:
            app.logger.warning(
                "City/neighborhood combination failed validation: %s/%s (confidence: %.2f)",
                city,
                neighborhood,
                confidence,
            )
            ...
            try:
                ...
                fallback_guide = SynthesisEnhancer.generate_neighborhood_paragraph(
                    neighborhood, city
                )
                return jsonify(
                    {
    ...
    def _looks_like_disambiguation_text(txt: str) -> bool:
        ...
        bullet_like = sum(
            1
            for l in lines
            if l.startswith("*") or l.startswith("•") or (len(l.split()) < 6 and "," in l)
        )
        if bullet_like >= 3:
            return True
        return False
    ...
    def _page_is_relevant(j: dict) -> bool:
        ...
        if is_country:
            ...
            if (city_lower in extract_text or city_lower in title_text) and (
                neighborhood_lower in extract_text or neighborhood_lower in title_text
            ):
                return True
        else:
        ...
        if neighborhood_lower in extract_text or neighborhood_lower in title_text:
            ...
            if any(ev in title_text for ev in event_keywords):
                if city.lower() not in extract_text and city.lower() not in title_text:
                    app.logger.debug(
                        "Rejecting page with neighborhood in title but event-like content: %s",
                        title_text,
                    )
                    return False
                else:
    ...
    from city_guides.src.snippet_filters import (
        looks_like_ddgs_disambiguation_text as _looks_like_ddgs_disambiguation_text,
    )
        ...
        try:
            ...
            async def _run_query(q):
                async with sem:
                    if not ddgs_search:
                        app.logger.debug(
                            "DDGS provider not available at runtime; skipping query %s", q
                        )
                        return []
                    try:
                        res = await ddgs_search(
                            q,
                            engine="google",
                            max_results=3,
                            timeout=float(os.getenv("DDGS_TIMEOUT", "5")),
                        ...
                        app.logger.info(
                            'DDGS: query="%s" got %d results', q, len(res) if res else 0
                        )
                        if res:
                            for i, r in enumerate(res[:3]):  # Log first 3 results
                                app.logger.info(
                                    'DDGS result %d: title="%s" body="%s"',
                                    i,
                                    (r.get("title") or "")[:100],
                                    (r.get("body") or "")[:100],
                                )
            ...
            try:
                blocked_domains = [
                    d.strip().lower()
                    for d in os.getenv(
                        "BLOCKED_DDGS_DOMAINS", "tripsavvy.com,tripadvisor.com"
                    ).split(",")
                    if d.strip()
                ]
                from city_guides.src.snippet_filters import filter_ddgs_results

                allowed_results, blocked_results = filter_ddgs_results(
                    ddgs_results, blocked_domains
                )
                if blocked_results:
                    app.logger.info(
                        "Blocked %d DDGS results for domains: %s",
                        len(blocked_results),
                        ",".join(
                            sorted(
                                set(
                                    [
                                        (r.get("href") or r.get("url") or "").split("/")[2]
            ...
            for r in ddgs_results:
                ...
                if href and any(
                    h in href.lower()
                    for h in ["youtube.com", "facebook.com", "instagram.com", "tiktok.com"]
                ):
                ...
                if _looks_like_ddgs_disambiguation_text(txt):
                    app.logger.debug(
                        "Filtered DDGS candidate as disambiguation/promotional: %s",
                        (r.get("title") or "")[:120],
                    )
                    continue
                ...
                mentions_neighborhood = (
                    neighborhood.lower() in lower or neighborhood.lower() in title_lower
                )
                ...
                is_substantial = (
                    ...
                    and not any(
                        irrelevant_topic in lower
                        for irrelevant_topic in [
                ...
                app.logger.info(
                    "DDGS filtering for %s/%s: mentions_neighborhood=%s, is_substantial=%s, is_generic_promo=%s",
                    city,
                    neighborhood,
                    mentions_neighborhood,
                    is_substantial,
            ...
            if not relevant and ddgs_results:
                app.logger.debug(
                    "No clearly relevant DDGS hits for %s/%s but using top search results",
                    city,
                    neighborhood,
                )
            ...
            if relevant:
                app.logger.debug(
                    "DDGS candidates for %s/%s: %s",
                    city,
                    neighborhood,
                    [(r.get("title"), r.get("href") or r.get("url")) for r in relevant],
                )
                ...
                if (
                    len(combined_text) >= 50
                    and (
                        neighborhood.lower() in combined_text.lower()
                        or city.lower() in combined_text.lower()
                    )
                    and not _looks_like_ddgs_disambiguation_text(combined_text)
                ):
                    ...
                    source_url = href
                    app.logger.info(
                        "Used DDGS result directly for %s/%s with attribution", city, neighborhood
                    )
                else:
                    ...
                    if chosen:
                        ...
                        try:
                            ...
                            original_combined = " ".join(
                                [(r.get("body") or "") for r in relevant[:4]]
                            )
                            fallback = f"{neighborhood} is a neighborhood in {city}."
                            synthesized = SynthesisEnhancer.ensure_includes_term(
                                synthesized,
                                original_combined,
                                neighborhood,
                            ...
                            app.logger.exception(
                                "Failed to ensure neighborhood inclusion in DDGS fallback"
                            )
                        source = "ddgs"
                        ...
                        for key in ("quick_guide", "quickGuide", "summary", "description"):
                            if key in cj and cj[key]:
                                ...
                                if neighborhood.lower() in txt.lower():
                                    ...
                                    matched = [
                                        s for s in parts if neighborhood.lower() in s.lower()
                                    ]
                                    if matched:
    ...
    if not synthesized:
        ...
        try:
            ...
            if not ddgs_search:
                app.logger.debug(
                    "DDGS provider not available at runtime; skipping DDGS fallback for %s/%s",
                    city,
                    neighborhood,
                )
            else:
                    ...
                    try:
                        ...
                        for r in results:
                            ...
                            if len(text) >= 60 and (
                                ...
                                or neighborhood.lower() in text.lower()
                                or q.lower().startswith(city.lower())
                            ):
                                ddgs_snippet = text
            ...
            if ddgs_snippet:
                ...
                if href and any(
                    ...
                    app.logger.debug(
                        "Filtered ddgs_snippet as disambiguation/promotional content: %s",
                        ddgs_snippet[:120],
                    )
                    ddgs_snippet = None
            ...
            if ddgs_snippet:
                ...
                try:
                    ...
                    fallback = f"{neighborhood} is a neighborhood in {city}."
                    ddgs_snippet = SynthesisEnhancer.ensure_includes_term(
                        ddgs_snippet,
                        ddgs_original,
                        neighborhood,
                        fallback_sentence=fallback,
                ...
                if (
                    city.lower() in ddgs_snippet.lower()
                    or neighborhood.lower() in ddgs_snippet.lower()
                    or (
                        ddgs_url
                        and (
                            neighborhood.lower() in (ddgs_url or "").lower()
                            or city.lower() in (ddgs_url or "").lower()
                        )
                    )
                ):
                    ...
                    app.logger.debug(
                        "DDGS snippet rejected for %s/%s: does not mention city/neighborhood",
                        city,
                        neighborhood,
                    )
        except Exception:
        ...
        app.logger.info(
            "Reached synthesis fallback for %s/%s, synthesized=%s",
            city,
            neighborhood,
            bool(synthesized),
        )
        if not synthesized:
            try:
                ...
                app.logger.info(
                    "About to call SynthesisEnhancer.generate_neighborhood_paragraph for %s/%s",
                    city,
                    neighborhood,
                )
                ...
                source = "synthesized"
                app.logger.info(
                    "Successfully generated synthesized paragraph for %s/%s: %s",
                    city,
                    neighborhood,
                    synthesized[:100],
                )
                ...
                app.logger.exception(
                    "SynthesisEnhancer failed for %s/%s: %s", city, neighborhood, str(e)
                )
    ...
    try:
        ...
        synthesized = SynthesisEnhancer.neutralize_tone(
            synthesized or "", neighborhood=neighborhood, city=city, max_length=400
        )
    except Exception:
    ...
    try:
        ...
        if (
            looks_like_ddgs_disambiguation_text(synthesized or "")
            or "missing:" in (synthesized or "").lower()
        ):
            app.logger.info(
                "Rejecting synthesized quick_guide for %s/%s as disambiguation/promotional content; using generic fallback",
                city,
                neighborhood,
            )
            synthesized = f"{neighborhood} is a neighborhood in {city}."
    ...
    try:
        from city_guides.providers.wikipedia_neighborhood_provider import (
            wikipedia_neighborhood_provider,
        )

        wiki_data = await wikipedia_neighborhood_provider.get_neighborhood_data(
            city, neighborhood, aiohttp_session
        )
        if wiki_data:
            ...
            if wikipedia_enhancement:
                app.logger.info(
                    f"Wikipedia data found for {city}/{neighborhood}: {wikipedia_enhancement[:100]}"
                )
    except Exception as e:
    ...
    groq_enhancement = ""
    if not wikipedia_enhancement and _is_content_sparse_or_low_quality(
        synthesized, neighborhood, city
    ):
        try:
            from city_guides.providers.groq_neighborhood_provider import groq_neighborhood_provider

            groq_data = await groq_neighborhood_provider.generate_neighborhood_content(
                city, neighborhood, aiohttp_session
            )
            if groq_data:
                ...
                if groq_info.get("description"):
                    ...
                    app.logger.info(
                        f"Groq enhanced sparse content for {city}/{neighborhood}: {groq_enhancement[:100]}"
                    )
        except Exception as e:
    ...
    if (
        not wikipedia_enhancement and not groq_enhancement
    ):  # Only try Teleport if both Wikipedia and Groq failed
        try:
    ...
    try:
        ...
        if mapillary_provider:
            ...
            if latlon and latlon.get("lat"):
                try:
                    imgs = await mapillary_provider.async_search_images_near(
                        latlon["lat"], latlon["lon"], radius_m=400, limit=6, session=aiohttp_session
                    )
                    for it in imgs:
                        mapillary_images.append(
                            {
    ...
    try:
        ...
        cleaned_quick_guide, image_attributions = SynthesisEnhancer.extract_image_attributions(
            synthesized or ""
        )
    ...
    if not mapillary_images:
        ...
        if image_provider:
            try:
                ...
                try:
                    wik_img = await image_provider.fetch_banner_from_wikipedia(
                        f"{neighborhood}, {city}"
                    )
                except Exception:
                    wik_img = None
                ...
                if wik_img and (wik_img.get("remote_url") or wik_img.get("url")):
                    if _is_relevant_wikimedia_image(wik_img, city, neighborhood):
                        ...
                        app.logger.info(
                            "Skipping wikimedia image based on relevance heuristic: %s",
                            wik_img.get("page_title") or wik_img.get("remote_url"),
                        )
                # include any attributions found in the quick_guide text as metadata (deduped)
                for a in image_attributions:
                    if not any(
                        (a.get("url") and a.get("url") == m.get("source_url"))
                        for m in mapillary_images
                    ):
                        mapillary_images.append(
                            {
                                "id": None,
                                ...
                                "attribution": SynthesisEnhancer.create_attribution(
                                    a.get("provider"), a.get("url")
                                ),
    ...
    try:
        ...
        if out.get("source") == "ddgs" and looks_like_ddgs_disambiguation_text(
            out.get("quick_guide") or ""
        ):
            app.logger.info(
                "Not caching disambiguation/promotional ddgs quick_guide for %s/%s",
                city,
                neighborhood,
            )
            ...
            try:
                ...
                out["quick_guide"] = SynthesisEnhancer.generate_neighborhood_paragraph(
                    neighborhood, city
                )
        ...
        try:
            ...
            out["quick_guide"] = SynthesisEnhancer.neutralize_tone(
                out.get("quick_guide") or "", neighborhood=neighborhood, city=city
            )
        except Exception:
...
async def prewarm_rag_responses(top_n: int = None):
    ...
    try:
        ...
        cities = sorted(cities, key=lambda c: int(c.get("population", 0) or 0), reverse=True)[
            :top_n
        ]
                    ...
                    try:
                        ...
                        try:
                            ...
                            if existing:
                                await redis_client.expire(
                                    ck, int(os.getenv("RAG_CACHE_TTL", 60 * 60 * 6))
                                )
                                continue
                        except Exception:
                            pass
                        ...
                        try:
                            async with aiohttp_session.post(
                                f"http://localhost:5010/api/chat/rag", json=payload, timeout=15
                            ) as resp:
                                if resp.status != 200:
                                    app.logger.debug(
                                        "Prewarm RAG failed for %s/%s: status %s",
                                        city_name,
                                        q,
                                        resp.status,
                                    )
                            ...
                            app.logger.debug(
                                "Prewarm RAG http failed for %s/%s: %s", city_name, q, exc
                            )
                            continue
                        ...
                        if data and isinstance(data, dict):
                            ...
                            try:
                                ...
                                app.logger.debug(
                                    "Failed to set RAG cache for %s/%s: %s", city_name, q, exc
                                )
                    except Exception as exc:
    ...
    try:
        ...
        try:
            neighborhoods = await multi_provider.async_get_neighborhoods(
                city=city, lang=lang, session=aiohttp_session
            )
        except Exception:
            neighborhoods = []
        if neighborhoods:
            ...
            try:
                ...
                if recommender.api_key:
                    messages = [
                        {
                            "role": "system",
                            "content": "Extract location information from travel queries. Return JSON with city, country, and optionally state/neighborhood. Be conservative - only return locations you're confident about.",
                        },
...
async def location_suggestions():
    ...
    try:
        ...
        all_locations.sort(
            key=lambda x: (not x["exact_match"], -x["weight"], len(x["display_name"]))
        )
...
async def geonames_search():
    ...
    try:
        ...
        async with get_session() as session:
            ...
            async with session.get(
                "http://api.geonames.org/searchJSON", params=params, timeout=10
            ) as response:
                ...
                for geoname in data.get("geonames", []):
                    ...
                    if feature_code in ["PPLX", "PPLQ", "PPLH"]:
                        app.logger.debug(
                            f"Skipping suburb/abandoned place: {city_name} ({feature_code})"
                        )
                        continue

                    # Skip Lyon suburbs (communes in Lyon metro area) - tourists want Lyon
                    if country_code == "FR" and city_name in [
                        "Villeurbanne",
                        "Bron",
                        "Vénissieux",
                        "Saint-Priest",
                    ...
                    photos.append(
                        {
                            ...
                            "links": {"unsplash": photo["links"]["html"]},
                        }
                    )
