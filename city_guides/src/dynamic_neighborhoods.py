...
async def fetch_neighborhoods_dynamic(city: str, lat: float, lon: float, radius: int = 15000) -> List[Dict]:
    """
    Dynamically fetch neighborhoods for ANY city using Overpass API
    No hardcoded lists - purely API-driven
    ...
    # Overpass API query to find neighborhoods, districts, and quarters
    overpass_query = f"""
    [out:json][timeout:25];
    (
      // Find neighborhoods (quarter), districts, and suburbs
      ...
      // Also look for administrative boundaries that are neighborhoods
      node["boundary"="administrative"]["admin_level"="9"](around:{radius},{lat},{lon});
      way["boundary"="administrative"]["admin_level"="9"](around:{radius},{lat},{lon});
      relation["boundary"="administrative"]["admin_level"="9"](around:{radius},{lat},{lon});
      
      // Look for named areas
      node["name"]["place"="locality"](around:{radius},{lat},{lon});
    );
    out center tags 20;
    """
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    logger.warning(f"Overpass API returned {response.status} for {city}")
                    return []
        ...
        logger.warning(f"Timeout fetching neighborhoods for {city}")
        return []
...
async def get_neighborhoods_for_city(city: str, lat: float, lon: float) -> List[Dict]:
    ...
    Tries Overpass first, then Wikidata as fallback
    """
