...
def discover_restaurants(city: str, limit: int = 50, cuisine: str = None) -> List[Dict]:
    ...
    for itm in j.get('features', [])[:limit]:
        ...
        if xid:
            try:
                dd = requests.get(f"{BASE}/xid/{xid}", params={'apikey': OPENTRIPMAP_KEY}, timeout=15)
                dd.raise_for_status()
                detail = dd.json()
            except Exception:
                detail = {}
        ...
        addr = ', '.join([address.get(k, '') for k in ('road', 'house_number', 'city', 'state', 'country') if address.get(k)])
        osm_url = ''
        if detail.get('url'):
        ...
        entry = {
            ...
            'osm_url': osm_url,
            'tags': ', '.join([f"{k}={v}" for k, v in (detail.get('properties', {}) or {}).items()]) if detail.get('properties') else '',
