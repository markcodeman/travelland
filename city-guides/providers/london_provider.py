...
def fetch_overpass_transit(lat, lon, radius=1500):
    ...
    r = requests.post(url, data=q.encode("utf-8"), headers={**HEADERS, "Content-Type": "text/plain"}, timeout=30)
    r.raise_for_status()
    data = r.json()
    stops = []
...
def fetch_opentripmap_pois(lat, lon, radius=1000, apikey=None, limit=20):
    ...
    try:
        ...
        params = {"radius": radius, "lon": lon, "lat": lat, "apikey": apikey, "limit": limit}
        r = requests.get(url, params=params, headers=HEADERS, timeout=10)
        r.raise_for_status()
        res = r.json()
        pois = []
